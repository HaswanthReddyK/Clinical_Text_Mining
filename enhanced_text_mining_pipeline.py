...
"""
=============================================================================
ENHANCED KP TEXT MINING PIPELINE  (v3 — 10/10 Edition)
=============================================================================
Intelligence upgrades over v2
------------------------------
Accuracy
  * Curated seed ICD table — 50+ direct entity-text -> ICD-10 mappings for
    all supported condition categories. Seed hits score 1.0 and bypass TF-IDF.
  * Label-filtered TF-IDF — when scikit-learn is available, each entity label
    restricts the cosine search to its relevant ICD-10 chapter (e.g. DIABETES
    only searches E10-E13, not 98k codes). Eliminates false matches like
    "family history of diabetes" for a current diabetes diagnosis.
  * CKD stage extraction — detects "stage 3" / "CKD4" text near a
    KIDNEY_DISEASE entity and maps directly to N181-N186.
  * eGFR-to-stage inference — extracts a numeric eGFR value (e.g. "eGFR 22")
    and infers CKD stage code automatically (22 -> N184 Stage 4).
  * Temporal qualifier detection — "history of X", "prior X", "previous X"
    inline markers add temporal="HISTORICAL" to the entity so downstream
    consumers can distinguish current vs historical conditions.
  * Negation is clause-aware (v2); kept here unchanged.
  * Section context (v2); kept here unchanged.

Flexibility
  * --auto-install flag: missing optional packages are pip-installed and the
    script restarts automatically so all features are enabled in one command.
  * --exclude-historical flag: filter out temporally historical inline entities.

Robustness
  * Comprehensive ICD seed validation at startup — seeds referencing non-existent
    codes are silently skipped; a DEBUG log lists any misses.
  * All v2 robustness features retained.

Scalability
  * Per-label TF-IDF sub-matrices built once at startup; query cost is O(subset)
    instead of O(98k) codes.
  * All v2 scalability features retained (nlp.pipe, pre-compiled regex, thread pool).

Extension guide:
  1. MedicalEntityExtractor._RULER_ENTRIES — spaCy patterns     <- ADD HERE (1/3)
  2. MedicalEntityExtractor._REGEX          — regex fallback    <- ADD HERE (2/3)
  3. ClinicalSpellChecker.PROTECTED         — medical terms     <- ADD HERE (3/3)
  4. ICD10Mapper._SEEDS                     — direct mappings   <- ADD HERE (4/4)
=============================================================================
"""
...
# =============================================================================
# DATABRICKS CONFIGURATION  (uncomment for Option B)
# =============================================================================
# from typing import Iterator
# from databricks import sql as dbsql
# DATABRICKS_HOST  = "<workspace>.azuredatabricks.net"
# DATABRICKS_TOKEN = "<token>"
# DATABRICKS_HTTP  = "/sql/1.0/warehouses/<id>"
# TABLE_NAME  = "catalog.schema.clinical_table"
# TEXT_COLUMN = "RESULT_TEXT"
# BATCH_SIZE  = 500
# ANCHOR_COL  = "DOCUMENT_ID"

# def fetch_from_databricks(query, batch_size=BATCH_SIZE, max_retries=3, last_anchor=None):
#     with dbsql.connect(...) as conn:
#         with conn.cursor() as cur:
#             while True:
#                 keyset = f"AND {ANCHOR_COL} > {last_anchor!r}" if last_anchor else ""
#                 cur.execute(f"{query} {keyset} ORDER BY {ANCHOR_COL} LIMIT {batch_size}")
#                 rows = cur.fetchall()
#                 if not rows: break
#                 batch = [dict(zip([d[0] for d in cur.description], r)) for r in rows]
...
class MedicalEntityExtractor:
    ...
    _REGEX: List[Tuple[str, str]] = [
        ...
        ("KIDNEY_DISEASE",   r"\b(?:end[\s\-]+stage\s+renal|esrd|chronic\s+kidney\s+disease|ckd)\b"),
        ("DIALYSIS",         r"\b(?:hemo|peritoneal\s+)?dialysis\b"),
        ("KIDNEY_FUNCTION",  r"\b(?:e?gfr|glomerular\s+filtration\s+rate?|creatinine)\b"),
...
if __name__ == "__main__":
    ...
    # =========================================================================
    # OPTION B — Databricks (uncomment and configure block at top)
    # =========================================================================
    ...
    # QUERY = f"SELECT {ANCHOR_COL} AS DOCUMENT_ID, CONTACT_LINE, CONTACT_DATE, {TEXT_COLUMN} AS RESULT_TEXT FROM {TABLE_NAME} WHERE {TEXT_COLUMN} IS NOT NULL"
    # for batch in fetch_from_databricks(QUERY, last_anchor=last_anchor):
    #     batch_number += 1
    #     results = pipeline.run(batch)
    #     total_processed += len(batch); total_flagged += len(results)
    #     is_first = (batch_number == 1 and last_anchor is None)
    #     if results:
    #         export_csv(results,         filepath=str(out/"enhanced_results.csv"),   append=not is_first)
    #         export_sql_inserts(results, filepath=str(out/"enhanced_results.sql"),   append=not is_first)
    #         export_json(results,        filepath=str(out/"enhanced_results.jsonl"), append=not is_first)
    #     ckpt.save(batch[-1][ANCHOR_COL], batch_number, total_processed)
    # ckpt.clear()
    # log.info(f"Complete. Processed: {total_processed:,}  Flagged: {total_flagged:,}")
